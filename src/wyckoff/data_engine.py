...
class DataEngine:
    ...
    def run(
        ...
        trading_plan = self._step5_trading_plan(
            bc_result, phase_result, effort_result, phase_c_test, counterfactual, risk_assessment
        )

        # 计算置信度
        confidence = self._calc_confidence(
            bc_result, phase_result, phase_c_test, counterfactual, risk_assessment
        )
        ...
        return DailyRuleResult(
            ...
            abandon_reason=""
            if trading_plan.direction != "abandon"
            else "unfavorable_rr_or_structure",
        )
    ...
    def _step_preprocess(self, df: pd.DataFrame) -> PreprocessingResult:
        ...
        for i in range(20, len(df)):
            ...
            if df["low"].iloc[i] == rolling_low.iloc[i]:
                local_lows.append(
                    {"index": i, "date": str(df["date"].iloc[i].date()), "price": df["low"].iloc[i]}
                )
    ...
    def _step2_effort_result(
        ...
        for anomaly in prep.limit_anomalies:
            if anomaly["type"] == "limit_up":
                ...
                if idx > len(df) - 20:
                    # 最近 20 日有涨停
                    post_limit_pct = (df["close"].iloc[-1] - df["close"].iloc[idx]) / df[
                        "close"
                    ].iloc[idx]
    ...
    def _step4_risk_assessment(
        ...
        if phase_c_test.spring_detected and phase_c_test.spring_date:
            ...
            freeze_until = str(
                (spring_dt + timedelta(days=self.rule_config.spring_freeze_days)).date()
            )
