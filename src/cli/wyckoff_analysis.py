...
def _save_all_outputs(
    ...
    raw_analysis = {
        ...
        "structure": {
            ...
            if hasattr(report.structure.phase, "value")
            else str(report.structure.phase),
            ...
            "sc_point": {
                "date": report.structure.sc_point.date
                if report.structure and report.structure.sc_point
                else None,
                "price": report.structure.sc_point.price
                if report.structure and report.structure.sc_point
                else None,
            },
        ...
        "multi_timeframe": {
            ...
            "constraint_note": report.multi_timeframe.constraint_note
    ...
    with open(
        os.path.join(output_dirs["raw"], f"analysis_{symbol_slug}_{timestamp}.json"),
        "w",
        encoding="utf-8",
    ) as f:
    ...
    if image_evidence and hasattr(image_evidence, "files") and image_evidence.files:
        ...
        with open(
            os.path.join(output_dirs["raw"], f"image_evidence_{symbol_slug}_{timestamp}.json"),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    summary_data = [
        ...
        [
            ...
            analysis_date,
            report.structure.phase.value
            if report.structure and hasattr(report.structure.phase, "value")
            else "unknown",
            report.signal.signal_type if report.signal else "N/A",
            analysis_result.decision
            if analysis_result
            else (report.trading_plan.direction if report.trading_plan else "N/A"),
            ...
            analysis_result.rr_assessment
            if analysis_result
            else (
                "pass"
                if report.risk_reward and report.risk_reward.reward_risk_ratio >= 2.5
                else "fail"
            ),
            analysis_result.t1_risk_assessment
            if analysis_result
            else (
                report.signal.t1_risk评估 if report.signal and report.signal.t1_risk评估 else "N/A"
            ),
    ...
    with open(
        os.path.join(output_dirs["summary"], f"analysis_summary_{symbol_slug}_{timestamp}.csv"),
        "w",
        encoding="utf-8",
        newline="",
    ) as f:
    ...
    if analysis_result and hasattr(analysis_result, "conflicts") and analysis_result.conflicts:
        ...
        with open(
            os.path.join(output_dirs["evidence"], f"{symbol_slug}_conflicts_{timestamp}.json"),
            "w",
            encoding="utf-8",
        ) as f:
...
def main() -> None:
    ...
    parser.add_argument(
        "--mode",
        choices=["data-only", "images-only", "fusion"],
        default="data-only",
        help="分析模式",
    )
    parser.add_argument(
        "--multi-timeframe", action="store_true", help="使用日线合成周线/月线进行多周期分析"
    )
    ...
    if args.mode in ["images-only", "fusion"] and (args.chart_dir or args.chart_files):
        ...
        image_engine.generate_chart_manifest_json(
            manifest,
            os.path.join(
                output_dirs["evidence"], f"{args.symbol.replace('.', '_')}_chart_manifest.json"
            ),
        )

        logger.info(
            f"图像扫描完成：{len(manifest['files'])} 张图片，时间周期：{image_evidence.detected_timeframe}"
        )
    ...
    if args.mode == "fusion":
        ...
        analysis_result = fusion_engine.fuse(report=report, image_evidence=image_evidence)
        ...
        state_manager.update_state(
            ...
            output_path=os.path.join(
                output_dirs["state"], f"{args.symbol.replace('.', '_')}_wyckoff_state.json"
            ),
        )
        logger.info(
            f"融合分析完成：决策={analysis_result.decision}, 置信度={analysis_result.confidence}"
        )
    ...
    if analysis_result:
        report_lines_output.extend(
            [
                "\n" + "=" * 60,
                "融合分析结果",
                "=" * 60,
                f"决策：{analysis_result.decision}",
                f"置信度：{analysis_result.confidence}",
                f"触发条件：{analysis_result.trigger}",
                f"失效位：{analysis_result.invalidation}",
                f"目标位：{analysis_result.target_1}",
                "=" * 60,
            ]
        )
    for line in report_lines_output:
        logger.info(line)
