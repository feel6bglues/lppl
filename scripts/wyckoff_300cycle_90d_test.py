...
def generate_cycle_specs(seed: int = 42) -> List[CycleSpec]:
    """生成 300 组随机年份和日期的测试规格（2012-2026）"""
    random.seed(seed)
    specs = []
    # 从2012-2026随机选择300个年份（允许重复）
    years = sorted([random.choice(range(2012, 2027)) for _ in range(300)])
...
def write_outputs(output_dir: Path, results: List[Dict], analysis: Dict) -> None:
    ...
    with (output_dir / "cycle300_raw_results.jsonl").open("w", encoding="utf-8") as f:
        for row in results:
            f.write(json.dumps(convert_keys_to_str(row), ensure_ascii=False) + "\n")

    if results:
        df = pd.DataFrame(results)
        df.to_csv(output_dir / "cycle300_results.csv", index=False, encoding="utf-8-sig")

    with (output_dir / "cycle300_analysis.json").open("w", encoding="utf-8") as f:
        json.dump(convert_keys_to_str(analysis), f, ensure_ascii=False, indent=2)

    fit = analysis.get('fit_analysis', {})
    
    md_lines = [
        "# 300-Cycle Wyckoff Engine Test Report (Optimal Config: d400_w120_m40, 90d Return)",
        "",
    ...
    (output_dir / "cycle300_report.md").write_text("\n".join(md_lines), encoding="utf-8")

    print(f"\n输出文件:")
    print(f"  - {output_dir / 'cycle300_raw_results.jsonl'}")
    print(f"  - {output_dir / 'cycle300_results.csv'}")
    print(f"  - {output_dir / 'cycle300_analysis.json'}")
    print(f"  - {output_dir / 'cycle300_report.md'}")


def main() -> None:
    """主函数"""
    output_dir = PROJECT_ROOT / "output" / "wyckoff_300cycle_90d_d400_w120_m40"
    csv_path = PROJECT_ROOT / "data" / "stock_list.csv"

    print("=" * 60)
    print("Wyckoff Engine v3.0 - 300 Cycle Test (Optimal Config: d400_w120_m40, 90d Return)")
    print("=" * 60)
