...
def _evaluate_candidate_worker(
    params: Tuple[int, int, int, int, int, float, float, int, int, float, float, int, float, int, float, int, int, int, int],
) -> Dict[str, object]:
    ...
    (
        ...
        atr_period, atr_ma_window,
        atr_low_threshold, atr_high_threshold,
    ...
    signal_config = _build_signal_config(
        ...
        bb_period=bb_period, bb_std=bb_std, bb_width_threshold=bb_width_threshold,
...
def _stage1(
    ...
    end_date: str,
    workers: int,
) -> Tuple[pd.DataFrame, Tuple[Tuple[int, int, int], ...]]:
    """Stage 1: MA combination screening."""
    candidate_params = [
        (ma_s, ma_m, ma_l, 14, 40, 1.00, 1.15, 20, 2.0, 0.08, 120, 1.0, 0.15, 120, 2, 10, 0)
    ...
    top_seeds = scored[["ma_short", "ma_mid", "ma_long"]].drop_duplicates().head(3)
...
def _stage2(
    ...
    start_date: str,
    end_date: str,
    top_seeds: Tuple[Tuple[int, int, int], ...],
    workers: int,
) -> Tuple[pd.DataFrame, Tuple[Tuple[int, int, int, int, int, float, float], ...]]:
    """Stage 2: ATR auxiliary parameters."""
    candidate_params = [
        (ma_s, ma_m, ma_l, atr_p, atr_w, atr_low, atr_high, 20, 2.0, 0.08, 120, 1.0, 0.15, 120, 2, 10, 0)
        for ma_s, ma_m, ma_l in top_seeds
    ...
    top_seeds2 = scored[["ma_short", "ma_mid", "ma_long", "atr_period", "atr_ma_window", "atr_low_threshold", "atr_high_threshold"]].drop_duplicates().head(2)
...
def _stage3(
    ...
    start_date: str,
    end_date: str,
    top_seeds: Tuple[Tuple[int, int, int, int, int, float, float], ...],
    workers: int,
) -> pd.DataFrame:
    """Stage 3: BB auxiliary parameters."""
    candidate_params = [
        (ma_s, ma_m, ma_l, atr_p, atr_w, atr_low, atr_high, bb_p, bb_s, bb_w, 120, 1.0, 0.15, 120, 2, 10, 0)
        for ma_s, ma_m, ma_l, atr_p, atr_w, atr_low, atr_high in top_seeds
        for bb_p, bb_s, bb_w in product(
            [20, 30],
            [1.5, 2.0, 2.5],
            [0.05, 0.08, 0.10],
        )
...
def _stage4(
    ...
    """Stage 4: Risk layer parameters."""
    seeds = stage3_df[
        ["ma_short", "ma_mid", "ma_long", "atr_period", "atr_ma_window",
         "atr_low_threshold", "atr_high_threshold", "bb_period", "bb_std", "bb_width_threshold"]
    ].drop_duplicates().head(2)

    candidate_params = [
        (int(s.ma_short), int(s.ma_mid), int(s.ma_long),
         int(s.atr_period), int(s.atr_ma_window),
         float(s.atr_low_threshold), float(s.atr_high_threshold),
         int(s.bb_period), float(s.bb_std), float(s.bb_width_threshold),
         rf_ma, rf_buf, dd_stop, dd_lb, 2, 10, 0)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MA收敛+ATR波动率策略V2参数调优 (MA金叉死叉为主)")
    ...
    parser.add_argument("--output", default="output/ma_convergence_v2_tuning")
    ...
    for symbol in symbols:
        ...
        print(
            f"[STAGE1-BEST] {symbol} MA{int(best_stage1['ma_short'])}/{int(best_stage1['ma_mid'])}/{int(best_stage1['ma_long'])} "
        ...
        print(
            ...
            f"low={float(best_stage2['atr_low_threshold']):.2f} high={float(best_stage2['atr_high_threshold']):.2f} "
        ...
        print(f"[STAGE3] {symbol} BB auxiliary", flush=True)
        ...
        print(
            f"[STAGE3-BEST] {symbol} BB{int(best_stage3['bb_period'])} "
            f"width_cap={float(best_stage3['bb_width_threshold']):.3f} "
    ...
    stage1_full_path = summary_dir / f"ma_convergence_v2_stage1_full_{stamp}.csv"
    stage1_best_path = summary_dir / f"ma_convergence_v2_stage1_best_{stamp}.csv"
    stage2_full_path = summary_dir / f"ma_convergence_v2_stage2_full_{stamp}.csv"
    stage2_best_path = summary_dir / f"ma_convergence_v2_stage2_best_{stamp}.csv"
    stage3_full_path = summary_dir / f"ma_convergence_v2_stage3_full_{stamp}.csv"
    stage3_best_path = summary_dir / f"ma_convergence_v2_stage3_best_{stamp}.csv"
    stage4_full_path = summary_dir / f"ma_convergence_v2_stage4_full_{stamp}.csv"
    stage4_best_path = summary_dir / f"ma_convergence_v2_stage4_best_{stamp}.csv"
    stage5_full_path = summary_dir / f"ma_convergence_v2_stage5_full_{stamp}.csv"
    stage5_best_path = summary_dir / f"ma_convergence_v2_stage5_best_{stamp}.csv"
    ...
    for row in stage5_best.sort_values("objective_score", ascending=False).to_dict("records"):
        report_lines.extend(
            [
                ...
                f"- BB: period={int(row['bb_period'])} std={float(row['bb_std']):.1f} width_cap={float(row['bb_width_threshold']):.3f}",
    ...
    report_path = reports_dir / f"ma_convergence_v2_tuning_report_{stamp}.md"
