#!/usr/bin/env python3
"""
三策略组合验证 v6 — Wyckoff + MA5/20 + Vol MeanRev
====================================================
策略1: Wyckoff v2+P3 (22天持有, 结构分析)      Sharpe 0.74
策略2: MA5/20金叉 (14天持有, 趋势跟踪)         Sharpe 0.66
策略3: Vol MeanRev (5-10天, 波动率均值回归)    预期Sharpe 0.60
...
# ===== S3: Vol MeanRev (波动率均值回归) =====
def trade_vol_mr(df, as_of_date):
    """
    波动率均值回归策略:
    1. 计算ATR(20)并获取其历史90%分位
    2. 当前ATR > 90%分位 → 波动率扩张, 买入(预期收缩)
    3. 持有至ATR收缩到50%分位以下或最多MR_HOLD_MAX天
    4. 退出: 止盈+3% or 止损-3% or 到期
    """
    a = pd.Timestamp(as_of_date)
    h = df[df["date"] <= a]
    if len(h) < 100: return None
    
    close_vals = h["close"].values.astype(float)
    high_vals = h["high"].values.astype(float)
    low_vals = h["low"].values.astype(float)
    
    # 计算ATR序列
    tr_vals = []
    for i in range(1, len(h)):
        tr = max(high_vals[i]-low_vals[i], abs(high_vals[i]-close_vals[i-1]), abs(low_vals[i]-close_vals[i-1]))
        tr_vals.append(tr)
    if len(tr_vals) < 40: return None
    
    # 当前ATR(20)
    current_atr = float(np.mean(tr_vals[-20:]))
    # 历史ATR序列(用于分位计算)
    hist_atr = [float(np.mean(tr_vals[j:j+20])) for j in range(len(tr_vals)-20)]
    if len(hist_atr) < 40: return None
    
    p90 = float(np.percentile(hist_atr, 90))
    p50 = float(np.percentile(hist_atr, 50))
    
    # 波动率扩张触发: 当前ATR > 历史90%分位
    if current_atr <= p90: return None
    
    entry = float(h.iloc[-1]["close"])
    fut = df[df["date"] > a].head(MR_HOLD_MAX)
    if len(fut) < 3: return None
    
    tp_target = entry * 1.03  # +3% 止盈
    sl_target = entry * 0.97  # -3% 止损
    exit_price = None
    exit_day = len(fut)
    ...
    tr = (exit_price - entry) / entry * 100
    return {"ret": round(tr, 2), "days": exit_day, "entry": round(entry, 2), "er": exit_reason}
...
def process_stock(args):
    ...
    try:
        ...
        for w in windows:
            ...
            w3=trade_vol_mr(df,w)
            if w3: trades.append({"strategy":"vol_mr","symbol":sym,"window":w,**w3})
    except: pass
    return trades
...
def run():
    print("="*70)
    print("三策略组合验证 v6: Wyckoff + MA5/20 + Vol MeanRev")
    print("="*70)
    ...
    print(f"\n总交易: {len(df)}笔")
    for s in ["wyckoff","ma_cross","vol_mr"]:
        print(f"  {s}: {sum(1 for t in all_trades if t['strategy']==s)}")

    # 单策略统计
    results={}
    for sn in ["wyckoff","ma_cross","vol_mr"]:
        sub=df[df["strategy"]==sn]
        if len(sub)>=5: results[sn]=compute_stats(sub)

    # 相关性矩阵
    corr_data={}
    for t1,t2 in [("wyckoff","ma_cross"),("wyckoff","vol_mr"),("ma_cross","vol_mr")]:
    ...
    # 组合夏普 (三策略等权)
    s_vals=[results[s]["sharpe"] for s in ["wyckoff","ma_cross","vol_mr"] if s in results]
    if len(s_vals)==3:
        r_names=["wyckoff","ma_cross","vol_mr"]
    ...
    for s_name in ["wyckoff","ma_cross","vol_mr"]:
