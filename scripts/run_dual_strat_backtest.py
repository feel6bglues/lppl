#!/usr/bin/env python3
"""
双策略组合验证 — Wyckoff + MA5/20金叉 (移除短期反转)
============================================================
基于三期分析发现: 短期反转已失效(2020-2025夏普-0.295, MC正值概率0%)
策略1: Wyckoff v2+P3
策略2: MA5/20金叉
...
OUTPUT_DIR = PROJECT_ROOT / "output" / "dual_strat_backtest_2020"
...
def trade_wyckoff(df, as_of_date, csi):
    ...
    peak = entry
    ts_p = None; half = False; s2 = False; ep = None; er = "max_hold"; hs = False; ht = False
    d_ = 0
    for _, rw in f.iterrows():
        d_ += 1; c, hi, lo = float(rw["close"]), float(rw["high"]), float(rw["low"])
        peak = max(peak, hi)
        if lo <= ss:
            ep = ss; er = "stop_loss"; hs = True; break
        if d_ <= 30 and not half and eff_t and hi >= eff_t:
            half = True; s1p = eff_t; s2 = True; ts_p = peak - atr_m * atr; ht = True; continue
        if d_ == 30:
            s2 = True; ts_p = peak - atr_m * atr
        if s2:
            t = peak - atr_m * atr
            ts_p = max(ts_p, t) if ts_p else t
            if lo <= ts_p:
                ep = ts_p; er = "trailing_stop"; break
            if d_ > ts_d and not half:
                ep = c; er = "time_stop"; break
        ep = c
    if not hs and d_ >= mh:
        ep = float(f.iloc[-1]["close"]); er = "max_hold"
    if half and ht:
...
def process_stock(args):
    ...
    try:
        ...
        for w in windows:
            ...
            if w1:
                trades.append({"strategy": "wyckoff", "symbol": sym, "window": w, **w1})
            w2 = trade_ma(df, w)
            if w2:
...
def run():
    ...
    csi = load_csi300()
    print(f"沪深300: {len(csi) if csi is not None else 0}行")
    windows = gen_windows(csi, N_WINDOWS)
    print(f"窗口: {len(windows)} (2020-2025, {N_WINDOWS}个随机)")
    all_trades = []
    ...
    corr_val = round(float(merged["ret_w"].corr(merged["ret_m"])), 3) if len(merged) >= 5 else 0.5
    corr_data = {"wyckoff_vs_ma_cross": {"corr": corr_val, "n": len(merged)}}

    # 双策略组合夏普
    s_vals = [results[s]["sharpe"] for s in ["wyckoff", "ma_cross"] if s in results]
    if len(s_vals) == 2:
    ...
    for sn in ["wyckoff", "ma_cross"]:
        r = results.get(sn)
        if r:
            print(f"  {sn:13s} {r['n']:>8d} {r['mean_ret']:>7.2f}% {r['median_ret']:>7.2f}% "
                  f"{r['std']:>7.2f}% {r['win_rate']:>5.1f}% {r['avg_days']:>5.1f}d {r['sharpe']:>7.3f}")

    print(f"\n相关性:")
    print(f"  wyckoff_vs_ma_cross: {corr_val:.3f} (n={len(merged)})")

    print(f"\n组合效果:")
    print(f"  双策略等权夏普: {combo:.3f}")
    print(f"  vs Wyckoff单: {results.get('wyckoff', {}).get('sharpe', 0):.3f}")
    print(f"  vs MA5/20单: {results.get('ma_cross', {}).get('sharpe', 0):.3f}")
    print(f"  提升vs单Wyckoff: {(combo / max(results.get('wyckoff', {}).get('sharpe', 0.01), 0.01) - 1) * 100:+.1f}%")
    print(f"  对比旧三策略(含反转)2020-2025: 1.597")
    ...
    print(f"\n{'=' * 70}")
    print("目标检查:")
    print(f"  {'✅' if combo >= 2.0 else '❌'} 双策略夏普 > 2.0 (当前: {combo:.3f})")
    for s_name in ["wyckoff", "ma_cross"]:
        r = results.get(s_name)
        if r:
            print(f"  {'✅' if r['sharpe'] > 0 else '❌'} {s_name} 夏普正值 ({r['sharpe']:.3f})")
    for sn in ["wyckoff", "ma_cross"]:
        m = mc.get(sn)
        if m:
            print(f"  {'✅' if m['p_pos'] == 100.0 else '❌'} {sn} MC 100%正向 ({m['p_pos']}%)")
