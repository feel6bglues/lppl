...
def generate_cycle_specs(seed: int = 42) -> List[CycleSpec]:
    """生成 10 组随机年份和日期的测试规格（2017-2025）"""
    random.seed(seed)
    specs = []
    years = sorted([random.choice(range(2017, 2026)) for _ in range(10)])
...
def main() -> None:
    """主函数"""
    output_dir = PROJECT_ROOT / "output" / "wyckoff_10cycle_best_config_d400_w120_m40"
