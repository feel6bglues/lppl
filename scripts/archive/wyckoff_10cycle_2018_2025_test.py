...
def generate_cycle_specs(seed: int = 42) -> List[CycleSpec]:
    """生成 10 组随机年份和日期的测试规格（2018-2025）"""
    random.seed(seed)
    specs = []
    # 从2018-2025随机选择10个年份（允许重复）
    years = sorted([random.choice(range(2018, 2026)) for _ in range(10)])
...
def process_single_stock(args: tuple) -> List[Dict]:
    ...
    try:
        ...
        # 使用400天日线，周线120行(折合600天)，月线40行(折合800天)
        engine = WyckoffEngine(lookback_days=lookback_days, weekly_lookback=120, monthly_lookback=40)
...
def main() -> None:
    """主函数"""
    output_dir = PROJECT_ROOT / "output" / "wyckoff_10cycle_2018_2025_test"
    csv_path = PROJECT_ROOT / "data" / "stock_list.csv"

    print("=" * 60)
    print("Wyckoff Engine v3.0 - 10 Cycle Test (2018-2025, 400 Days + Multi-timeframe)")
    print("=" * 60)
