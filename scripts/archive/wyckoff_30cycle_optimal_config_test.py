...
def generate_cycle_specs(seed: int = 42) -> List[CycleSpec]:
    """生成 30 组随机年份和日期的测试规格（2018-2025）"""
    random.seed(seed)
    specs = []
    # 从2018-2025随机选择30个年份（允许重复）
    years = sorted([random.choice(range(2018, 2026)) for _ in range(30)])
...
def write_outputs(output_dir: Path, results: List[Dict], analysis: Dict) -> None:
    ...
    with (output_dir / "cycle30_raw_results.jsonl").open("w", encoding="utf-8") as f:
        for row in results:
            f.write(json.dumps(convert_keys_to_str(row), ensure_ascii=False) + "\n")

    if results:
        df = pd.DataFrame(results)
        df.to_csv(output_dir / "cycle30_results.csv", index=False, encoding="utf-8-sig")

    with (output_dir / "cycle30_analysis.json").open("w", encoding="utf-8") as f:
    ...
    (output_dir / "cycle30_report.md").write_text("\n".join(md_lines), encoding="utf-8")

    print(f"\n输出文件:")
    print(f"  - {output_dir / 'cycle30_raw_results.jsonl'}")
    print(f"  - {output_dir / 'cycle30_results.csv'}")
    print(f"  - {output_dir / 'cycle30_analysis.json'}")
    print(f"  - {output_dir / 'cycle30_report.md'}")


def main() -> None:
    """主函数"""
    output_dir = PROJECT_ROOT / "output" / "wyckoff_30cycle_optimal_config_d400_w120_m40"
    csv_path = PROJECT_ROOT / "data" / "stock_list.csv"

    print("=" * 60)
    print("Wyckoff Engine v3.0 - 30 Cycle Test (Optimal Config: d400_w120_m40)")
    print("=" * 60)
