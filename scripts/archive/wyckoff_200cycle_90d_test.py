...
# -*- coding: utf-8 -*-
"""
200组周期测试 - 最优参数组合 (d400_w120_m40) + 90天收益
- 400天日线回看
...
def generate_cycle_specs(seed: int = 42) -> List[CycleSpec]:
    """生成 100 组随机年份和日期的测试规格（2012-2026）"""
    random.seed(seed)
    specs = []
    # 从2012-2026随机选择100个年份（允许重复）
    years = sorted([random.choice(range(2012, 2027)) for _ in range(100)])
...
def calculate_future_return(
    df: pd.DataFrame, as_of_date: str, days: int = 90
) -> Optional[Dict[str, float]]:
    """计算未来 N 个交易日的收益率（默认90天）"""
...
def process_single_stock(args: tuple) -> List[Dict]:
    ...
    try:
        ...
        for spec in cycle_specs:
            ...
            future_return = calculate_future_return(df, spec.as_of_date, days=90)
            ...
            results.append({
                ...
                "future_90d_return": future_return["return_pct"],
                "future_90d_max_gain": future_return["max_gain_pct"],
                "future_90d_max_drawdown": future_return["max_drawdown_pct"],
...
def analyze_results(results: List[Dict]) -> Dict:
    ...
    for phase in df["phase"].unique():
        ...
        phase_stats[phase] = {
            "count": len(phase_df),
            "avg_return": round(phase_df["future_90d_return"].mean(), 2),
            "median_return": round(phase_df["future_90d_return"].median(), 2),
            "win_rate": round(len(phase_df[phase_df["future_90d_return"] > 0]) / len(phase_df) * 100, 1),
            "avg_max_gain": round(phase_df["future_90d_max_gain"].mean(), 2),
            "avg_max_drawdown": round(phase_df["future_90d_max_drawdown"].mean(), 2),
        }
    ...
    for direction in df["direction"].unique():
        ...
        direction_stats[direction] = {
            "count": len(dir_df),
            "avg_return": round(dir_df["future_90d_return"].mean(), 2),
            "win_rate": round(len(dir_df[dir_df["future_90d_return"] > 0]) / len(dir_df) * 100, 1),
        }
    ...
    for conf in df["confidence"].unique():
        ...
        confidence_stats[conf] = {
            "count": len(conf_df),
            "avg_return": round(conf_df["future_90d_return"].mean(), 2),
            "win_rate": round(len(conf_df[conf_df["future_90d_return"] > 0]) / len(conf_df) * 100, 1),
        }
    ...
    for cycle_id in df["cycle_id"].unique():
        ...
        cycle_stats[cycle_id] = {
            ...
            "count": len(cycle_df),
            "avg_return": round(cycle_df["future_90d_return"].mean(), 2),
            "win_rate": round(len(cycle_df[cycle_df["future_90d_return"] > 0]) / len(cycle_df) * 100, 1),
        }
    ...
    for signal in df["signal_type"].unique():
        ...
        signal_accuracy[signal] = {
            "count": len(sig_df),
            "avg_return": round(sig_df["future_90d_return"].mean(), 2),
            "win_rate": round(len(sig_df[sig_df["future_90d_return"] > 0]) / len(sig_df) * 100, 1),
        }
    ...
    for alignment in df["mtf_alignment"].unique():
        if alignment:
            ...
            mtf_stats[alignment] = {
                "count": len(mtf_df),
                "avg_return": round(mtf_df["future_90d_return"].mean(), 2),
                "win_rate": round(len(mtf_df[mtf_df["future_90d_return"] > 0]) / len(mtf_df) * 100, 1),
            }
    ...
    return {
        ...
        "overall_avg_return": round(df["future_90d_return"].mean(), 2),
        "overall_median_return": round(df["future_90d_return"].median(), 2),
        "overall_win_rate": round(len(df[df["future_90d_return"] > 0]) / len(df) * 100, 1),
...
def analyze_signal_fit(df: pd.DataFrame) -> Dict:
    ...
    for phase in ["accumulation", "markup", "distribution", "markdown"]:
        ...
        if len(phase_df) > 0:
            ...
            actual_positive_rate = len(phase_df[phase_df["future_90d_return"] > 0]) / len(phase_df)
...
def write_outputs(output_dir: Path, results: List[Dict], analysis: Dict) -> None:
    ...
    with (output_dir / "cycle200_raw_results.jsonl").open("w", encoding="utf-8") as f:
        for row in results:
            f.write(json.dumps(convert_keys_to_str(row), ensure_ascii=False) + "\n")

    if results:
        df = pd.DataFrame(results)
        df.to_csv(output_dir / "cycle200_results.csv", index=False, encoding="utf-8-sig")

    with (output_dir / "cycle200_analysis.json").open("w", encoding="utf-8") as f:
    ...
    (output_dir / "cycle200_report.md").write_text("\n".join(md_lines), encoding="utf-8")

    print(f"\n输出文件:")
    print(f"  - {output_dir / 'cycle200_raw_results.jsonl'}")
    print(f"  - {output_dir / 'cycle200_results.csv'}")
    print(f"  - {output_dir / 'cycle200_analysis.json'}")
    print(f"  - {output_dir / 'cycle200_report.md'}")


def main() -> None:
    """主函数"""
    output_dir = PROJECT_ROOT / "output" / "wyckoff_100cycle_90d_d400_w120_m40"
    csv_path = PROJECT_ROOT / "data" / "stock_list.csv"

    print("=" * 60)
    print("Wyckoff Engine v3.0 - 100 Cycle Test (Optimal Config: d400_w120_m40, 90d Return)")
    print("=" * 60)
